...
export default async function Product({ data }: { data: Product }) {
  ...
  const conditionStyle = CONDITION_STYLES[ProductInfo.condition] ?? {
    label: ProductInfo.condition,
    text: "text-gray-600",
    bg: "bg-gray-200",
  };
  return (
    <div className="flex flex-col md:flex-row gap-6 my-6 sm:my-10 mx-4 sm:mx-6 md:mx-10">
      <div className="flex flex-col w-full md:w-auto">
        <Image
          ...
          width={100}
          height={100}
          className="border border-gray-500 rounded-2xl w-full md:w-100"
        />
        <div className="flex flex-wrap gap-2 sm:gap-4 my-3 justify-center md:justify-start">
      ...
      <div className="flex flex-col flex-1 min-w-0 w-full">
        ...
        <div className="flex flex-wrap gap-2 my-2">
          <div className="px-2 py-1">
            {" "}
            شارع {ProductInfo.location?.street}، مبني رقم{" "}
            {ProductInfo.location?.building}، دور {ProductInfo.location?.floor}،
            علامة مميزة {ProductInfo.location?.mark}
          </div>
          {ProductInfo.rating == null ? (
            <div className="flex items-center px-2 py-1 text-sm rounded-xl bg-[#FFFDFA]">
              0
              <svg
        ...
        <div className="border border-gray-300 bg-gray-50 px-3 py-3 rounded-2xl">
          ...
          <div className="grid grid-cols-3 gap-2 md:gap-3">
            {[
              ...
              <div
                key={unit}
                className="flex flex-col px-2 py-2 border border-gray-400 my-1 md:my-2 rounded-xl items-center justify-center min-h-20 md:h-30 hover:bg-brand-light hover:border hover:border-brand-primary">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border border-blue-950 bg-blue-100 rounded-xl px-2 py-3 my-2">
            <div className="flex items-center gap-1">
              <svg
                ...
                strokeLinecap="round"
                strokeLinejoin="round"
                className="icon icon-tabler icons-tabler-outline icon-tabler-shield text-blue-950 shrink-0">
                <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                <path d="M12 3a12 12 0 0 0 8.5 3a12 12 0 0 1 -8.5 15a12 12 0 0 1 -8.5 -15a12 12 0 0 0 8.5 -3" />
              </svg>
              <p>مبلغ التأمين يسترد بالكامل</p>
            </div>
            <p className="font-black mx-10">
              {ProductInfo.insuranceAmount} ج.م
            </p>
          </div>
        </div>
      </div>
